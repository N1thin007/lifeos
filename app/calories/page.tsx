...
import { INDIAN_FOODS, type IndianFood } from '@/lib/data/indianFoods'
import {
...
function AddFoodModal({
  ...
  const supabase = createClient()
  const [tab, setTab] = useState<'search' | 'indian' | 'manual' | 'saved'>('search')
  ...
  const [indianFilter, setIndianFilter] = useState('')
  ...
  function selectFood(food: SearchResult | Food | IndianFood) {
    setForm({
      name: food.name,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 sm:items-center" onClick={onClose}>
      ...
      >
        ...
        {/* Tabs */}
        <div className="mb-4 grid grid-cols-4 gap-2 rounded-xl bg-slate-800 p-1">
          {(['search', 'indian', 'saved', 'manual'] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
            ...
            >
              {t === 'saved' ? 'My recipes' : t === 'indian' ? 'Indian' : t}
            </button>
          ))}
        </div>
          ...
          <div>
            <div className="relative mb-3">
              ...
              <input
                autoFocus
                value={indianQuery}
                onChange={(e) => setIndianQuery(e.target.value)}
                placeholder="Search Indian dishes (e.g. dosa, biryani)"
                className="w-full rounded-xl border border-slate-700 bg-slate-800 py-2.5 pl-9 pr-3 text-sm text-white placeholder-slate-500 outline-none focus:border-indigo-500"
              />
            </div>
            <div className="max-h-80 space-y-2 overflow-y-auto">
              {INDIAN_FOODS.filter((f) =>
                f.name.toLowerCase().includes(indianQuery.toLowerCase())
              ).map((f, i) => (
                <button
                  key={i}
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {f.calories} kcal · {f.protein}g protein · {f.serving_size}
                    </p>
                  </div>
                  <Plus size={16} className="text-slate-500" />
                </button>
              ...
              {INDIAN_FOODS.filter((f) =>
                ...
                <p className="py-4 text-center text-sm text-slate-500">No matches. Try manual entry.</p>
              )}
            </div>
          </div>
        )}
